...
This protocol implements a peer-to-peer lending system for ERC20 tokens, available in two main versions: vaulted and securitize. Both allow ERC20 token owners to use their assets as collateral to borrow cryptocurrency, while lenders can provide loans and earn interest. The protocol is designed to be trustless, efficient, and flexible, with support for various ERC20 tokens as collateral. It also includes features for dynamic collateral management, partial liquidations, and loan refinancing. The vaulted version uses a vault system for enhanced compliance and isolated collateral management, while the securitize version adds support for Securitize DS Token collateral with a redemption workflow.
...
- `P2PLendingVaultCentrifugeAsync.vy`: A vault implementation for Centrifuge ERC-7540 AsyncVault shares — `MINT_ASYNC | MINT_CANCEL | REDEEM_ASYNC | REDEEM_CANCEL` (request-based mint/redeem with on-chain status and ERC-7887 cancellation).
...
1.  Support for any ERC20 token as collateral, specified at deployment (held within a dedicated vault for each borrower).
2.  Use of an ERC20 token (e.g., USDC) as a payment token, defined at deployment time for each instance of `P2PLendingVaultedErc20`.
...
9.  Dynamic collateral management (add/remove ERC20 collateral) is supported, managed through the borrower's dedicated vault.
...
*   **Vault capability flags**: Each vault implementation declares a constant capability bitmask (`MINT_SYNC`, `MINT_ASYNC`, `MINT_MANUAL`, `MINT_CANCEL`, `REDEEM_SYNC`, `REDEEM_ASYNC`, `REDEEM_MANUAL`, `REDEEM_CANCEL`). The lending contract reads it once at deployment into the `vault_capabilities` immutable and dispatches by capability — each flag maps 1:1 to a same-named vault function. There is no separate status flag: an `_ASYNC` vault is ERC-7540, which mandates the on-chain status views, so `*_status()` is always present when `_ASYNC` is set (off-chain async settlement without status is the `_MANUAL` mode instead). Every vault implements the full canonical `Vault` interface from `P2PLendingMultiVaultBase`; functions a vault does not support are stubs that revert. The `_CANCEL` flags are independent of `_ASYNC`: a vault may support async mint/redeem without ERC-7887 cancellation. When the upstream vault has **no cancel support**, there is no `cancel_pending_loan` / `cancel_redeem` path, so the protocol has a **higher dependency on the timely fulfilment of the mint/redeem requests** by the issuer — a request that is never fulfilled cannot be unwound on-chain and the position stays pending until it settles (the `max_pending_window` permissionless-cancel valve also requires cancel support). Prefer vaults that declare the matching `_CANCEL` flag where the upstream supports ERC-7887.
...
1.  **Offer Structure**: An offer is defined by the `Offer` structure (from `P2PLendingVaultedBase`), which includes:
...
*   Offers can be revoked before expiration by calling `revoke_offer` in `P2PLendingVaultedErc20`.
...
1.  **Loan Creation (`create_loan`)**:
    ...
    **For a loan to be successfully created, the lender must approve/hold funds for the total amount to be transferred from their wallet, which is calculated as `principal + protocol_upfront_fee_amount - origination_fee_amount`. If the lender's wallet does not hold sufficient funds or has not provided adequate allowance to the protocol for this calculated amount, the `create_loan` transaction will revert.**
    The ERC20 collateral is transferred to a dedicated `P2PLendingVault` for the borrower. If a vault doesn't exist for the borrower, it's created via CREATE2.
...
2.  **Loan Settlement (`settle_loan`)**:
    ...
    The ERC20 collateral is transferred from the borrower's `P2PLendingVault` back to the borrower.

3.  **Defaulted Loan Collateral Claim (`liquidate_loan`)**:
    If a loan defaults (either by reaching `maturity` or failing to repay within `call_window` after a `call_loan`), the lender or any address (liquidator) can trigger a full liquidation via `liquidate_loan` (handled by the `P2PLendingVaultedLiquidation` facet). The collateral is transferred from the borrower's `P2PLendingVault` to the lender (or liquidator for a fee) without any fund transfers.
...
4.  **Partial Liquidation (`partially_liquidate_loan`)**:
    If the current Loan-to-Value (LTV) ratio of an active loan exceeds the `partial_liquidation_ltv` threshold defined in the offer, any address can trigger a partial liquidation via `partially_liquidate_loan` (handled by the `P2PLendingVaultedLiquidation` facet). In this process:
...
6.  **Add Collateral (`add_collateral_to_loan`)**:
    Borrowers can add more ERC20 collateral to an ongoing loan at any time, which reduces the loan's LTV. Collateral is deposited into the borrower's `P2PLendingVault`.

7.  **Remove Collateral (`remove_collateral_from_loan`)**:
    Borrowers can remove collateral from an ongoing loan as long as the remaining collateral is at least `min_collateral_amount` and the LTV does not exceed the `initial_ltv` (to prevent immediately increasing risk beyond the initial agreement). Collateral is withdrawn from the borrower's `P2PLendingVault`.

8.  **Loan Replacement by Borrower (`replace_loan`)**:
    A borrower can refinance an existing loan by accepting a new offer (which might be from the same or a different lender). The function, handled by the `P2PLendingVaultedRefinance` facet, effectively settles the old loan and creates a new one using the same collateral. Liquidity adjustments are made for both borrower and lender, and any difference in collateral amount is transferred. KYC for the new lender is required. Collateral remains within the borrower's vault, with only internal adjustments if the amount changes.

9.  **Loan Replacement by Lender (`replace_loan_lender`)**:
    A lender can initiate a replacement of an existing loan, effectively selling it to a new lender or refinancing it themselves. This is handled by the `P2PLendingVaultedRefinance` facet. The borrower's terms are protected, ensuring:
...
The protocol uses the `P2PLendingVaultedErc20` contract with a dedicated vault system (`P2PLendingVault`) for each borrower to manage collateral, enhancing security and compliance. The Securitize version (`P2PLendingSecuritizeErc20`) extends this with DS Token support.

#### P2PLendingVaultedErc20 Contract (Vaulted) (`P2PLendingVaultedErc20.vy`)

The `P2PLendingVaultedErc20` contract serves as the main entry point for the vaulted protocol, integrating a dedicated vault system for collateral management. It inherits common state and functions from `P2PLendingVaultedBase`. Collateral interactions are handled through calls to the borrower's `P2PLendingVault`.
...
`P2PLendingVaultedBase.vy` provides common state and logic for the vaulted version. Its internal collateral handling functions (`_send_collateral`, `_receive_collateral`) interact with the `P2PLendingVault` system. It also provides functions like `_get_vault`, `_create_vault_if_needed`, and `_wallet_to_vault` to manage the lifecycle and address computation for borrower-specific vaults.

#### P2PLendingVaultedRefinance Contract (`P2PLendingVaultedRefinance.vy`)

This contract acts as a facet for refinancing logic. `P2PLendingVaultedRefinance.vy` takes `vault_impl_addr` as an additional parameter in its delegatecall context and internally uses `P2PLendingVaultedBase`'s vault-aware collateral functions.

#### P2PLendingVaultedLiquidation Contract (`P2PLendingVaultedLiquidation.vy`)

This contract acts as a facet for liquidation logic. `P2PLendingVaultedLiquidation.vy` takes `vault_impl_addr` as an additional parameter in its delegatecall context and internally uses `P2PLendingVaultedBase`'s vault-aware collateral functions.
...
The `P2PLendingVaultedErc20` and `P2PLendingSecuritizeErc20` contracts include support for authorized proxies, allowing for more flexible interaction with the protocol. This feature is particularly useful for integrations with other protocols or for implementing advanced user interfaces.
...
The protocol has comprehensive test suites for the vaulted and securitize versions. There are two types of tests implemented, running on py-evm using titanoboa:
...
The `P2PLendingVaultedErc20`, its Base contract, Refinance, and Liquidation facets are deployed in each environment. The main entry point `P2PLendingVaultedErc20` is deployed with the following parameters:
...
| `_refinance_addr` | `address` | The address of the `P2PLendingVaultedRefinance` facet contract. |
| `_liquidation_addr` | `address` | The address of the `P2PLendingVaultedLiquidation` facet contract. |
...
3.  Accept suitable offer with collateral transfer (collateral is deposited into your dedicated vault).
4.  Manage collateral position during loan term (all collateral operations go through your vault via the main lending contract).
...
-   Direct contract interactions with `P2PLendingVaultedErc20` (vaulted) or `P2PLendingSecuritizeErc20` (securitize).
