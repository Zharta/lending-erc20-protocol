...
def test_initial_state(
    ...
    assert p2p_usdc_weth.securitize_redemption_wallet() == securitize_redemption_wallet
...
# Securitize-specific tests for securitize_redemption_wallet
def test_set_securitize_redemption_wallet_reverts_if_not_owner(p2p_usdc_weth):
    random = boa.env.generate_address("random")
    new_wallet = boa.env.generate_address("new_wallet")
    with boa.reverts():
        p2p_usdc_weth.set_securitize_redemption_wallet(new_wallet, sender=random)


def test_set_securitize_redemption_wallet(p2p_usdc_weth, owner):
    new_wallet = boa.env.generate_address("new_wallet")
    p2p_usdc_weth.set_securitize_redemption_wallet(new_wallet, sender=owner)
    assert p2p_usdc_weth.securitize_redemption_wallet() == new_wallet


def test_set_securitize_redemption_wallet_logs_event(p2p_usdc_weth, owner, securitize_redemption_wallet):
    new_wallet = boa.env.generate_address("new_wallet")

    p2p_usdc_weth.set_securitize_redemption_wallet(new_wallet, sender=owner)
    event = get_last_event(p2p_usdc_weth, "SecuritizeRedemptionWalletChanged")

    assert event.old_wallet == securitize_redemption_wallet
    assert event.new_wallet == new_wallet
