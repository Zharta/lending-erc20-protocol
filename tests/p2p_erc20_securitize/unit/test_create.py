...
def test_create_loan_reverts_if_loan_already_exists(
    ...
    # For securitize, each loan gets a new vault. Approve vault for first loan.
    vault_addr_0 = p2p_usdc_weth.wallet_to_vault(borrower)
    weth.approve(vault_addr_0, collateral_amount, sender=borrower)
    usdc.mint(lender, principal * 2)
    ...
    # Even though vault_id would differ, the loan_id is computed from borrower, lender, start_time, offer_id
...
def test_create_loan_logs_event(
    ...
    assert event.vault_addr == p2p_usdc_weth.vault_id_to_vault(borrower, 0)
...
def test_create_loan_transfers_collateral_to_escrow(
    ...
    # Get vault address before creating loan (vault_count is still 0 at this point)
    ...
    # After create_loan, vault_count is 1, but the loan uses vault_id=0
    # Use vault_id_to_vault to get the correct vault address
...
def test_create_loan_for_different_lenders_track_liquidity_separately(
    ...
    usdc.mint(lender, principal)
    usdc.approve(p2p_usdc_weth.address, principal, sender=lender)
    usdc.mint(lender2, principal)
    usdc.approve(p2p_usdc_weth.address, principal, sender=lender2)

    # In Securitize, each loan creates a new vault with a new vault_id
    # We need to approve each vault separately before creating each loan
    # Get vault address for first loan (vault_count is 0)
    vault_addr_1 = p2p_usdc_weth.wallet_to_vault(borrower)
    weth.approve(vault_addr_1, collateral_amount, sender=borrower)
    p2p_usdc_weth.create_loan(signed_offer1, principal, collateral_amount, kyc_borrower, kyc_lender, sender=borrower)

    # After first loan, vault_count is 1, so second loan uses vault_id=1
    vault_addr_2 = p2p_usdc_weth.wallet_to_vault(borrower)
    weth.approve(vault_addr_2, collateral_amount, sender=borrower)
...
def test_create_vault_if_needed_registers_vault_with_registrar(
    p2p_usdc_weth, borrower, borrower_account, now, vault_registrar_weth, registrar_connector_weth, owner
):
    ...
    deadline = now + 3600
    v, r, s = sign_register_vault(borrower_account, registrar_connector_weth.address, vault_registrar_weth, deadline)
    registrar_connector_weth.set_investor_signature(deadline, (v, r, s), sender=borrower)
