...
Redeem side (redeem_sync): the slippage floor `min_receive_amount` is the base18 oracle-implied value
`amount_without_fee * oracle_rate_num // oracle_rate_den`. It must NOT be additionally divided by
`scale*scale` (scale = 10**(18 - token_out_decimals)), which would collapse the floor to ~0 for sub-18
decimal payment tokens like USDC and leave the slippage guard inert.

Mint side (mint_sync): the vault approves a MidasVaultMock deposit vault and calls `depositInstant`,
which pulls the stablecoin (capped by `set_max_deposit_spend` -> partial-spend/refund case) and pays
out `set_deposit_deliver_amount` mtoken from its own (pre-funded) balance, enforcing the base18 min-out
floor. The vault credits the delivered mtoken to `pending_transfers[owner]` and returns
...
def midas_vault(midas_vault_impl_def, mtoken, caller_addr, owner):
    ...
    meaningfully: driving mint_sync as `owner` must revert, only `caller_addr` is authorized.
    """
...
def test_redeem_sync_succeeds_with_6dec_token_out(midas_vault, redemption_mock, mtoken, token_out_usdc, caller_addr, owner):
    ...
    # the floor is the base18 oracle value, NOT collapsed by the 6-dec token_out scale
...
def test_redeem_sync_reverts_when_delivery_below_floor(
    ...
    """(2) Delivering one unit below the floor reverts.

    If the floor were divided by an extra (10**(18-6))**2 == 10**24 it would collapse to ~0 and this
    under-delivery would NOT revert (the slippage guard inert for USDC-like payment tokens).
    """
    amount_mtoken = 10**18
    num, den = 5, 1
...
def test_mint_sync_refunds_unspent_when_deposit_spend_capped(
    ...
    """A deposit-spend cap below the requested amount yields a refund (stable_coin_amount - spent)."""
...
# ============================================================================
# Unsupported-capability stubs (Midas is MINT_SYNC | REDEEM_SYNC; lacks manual + async paths)
# ============================================================================


def test_midas_unsupported_ops_revert(midas_vault, mtoken, caller_addr):
    """Every capability the Midas vault does NOT implement is a stub that `raise`s immediately (before any
    caller guard or state access), so any sender triggers the revert. Locks the exact strings.

    Midas supports mint_sync + redeem_sync; every other Vault entry point is an unsupported stub."""
    v = midas_vault
    dummy = boa.env.generate_address("dummy")

    with boa.reverts("redeem_manual not supported"):
        v.redeem_manual(dummy, mtoken.address, 0, 1, 1, sender=caller_addr)
    with boa.reverts("mint_manual not supported"):
        v.mint_manual(mtoken.address, dummy, 0, 0, sender=caller_addr)
    with boa.reverts("mint_async not supported"):
        v.mint_async(mtoken.address, dummy, 0, 0, sender=caller_addr)
    ...
    with boa.reverts("redeem_async not supported"):
        v.redeem_async(dummy, mtoken.address, 0, 1, 1, sender=caller_addr)
