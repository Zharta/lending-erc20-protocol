"""
Signature validation in `P2PLendingMultiVaultBase._is_offer_signed_by_lender` and
...
# Returns the magic value or a non-magic value depending on a settable flag, so a
# single mock covers both the valid and invalid contract-lender branches without
# needing a real owner signature.
EIP1271_MOCK_SRC = """
...
def _expected_loan(p2p, signed_offer, loan_id, now, borrower, lender, principal, collateral_amount):
    """The exact Loan a plain create_loan (no leverage) stores, for hash comparison."""
...
#
# The zero-address guard in `_is_extension_offer_signed_by_lender` ("invalid
# signature") is not reachable through the public `extend_loan` API with a zero
# lender: `extend_loan` passes `loan.lender` as the `lender` argument and asserts
# `_is_loan_valid(loan)` FIRST. Since `lender` is part of the loan hash, a loan with
# a zero lender fails `_is_loan_valid` ("invalid loan") before the extension guard is
# ever called. The guard is defence-in-depth; no public entry point reaches it with
# `lender == address(0)`.
#
# The test below pins the reachable behavior: a started loan whose lender field is
# zeroed reverts "invalid loan", not "invalid signature". A future check reorder that
# made the guard reachable would change this revert message and flag the shift.
...
# ============== 3. Normal EOA lender still validates ==============


def test_create_loan_eoa_lender_validates_via_plain_signature(
    p2p_usdc_weth, borrower, now, lender, lender_key, kyc_borrower, kyc_lender, usdc, weth, oracle
):
    """A normally-signed EOA offer creates successfully via `if signer == lender: return
    True` (plain ECDSA recover) before the ERC-1271 branch. An EOA has no code, so the
    recover path is the only one that can succeed here."""
    principal = 1000 * 10**6
    offer = _base_offer(now, lender, borrower, usdc, weth, oracle, principal)
    signed_offer = sign_offer(offer, lender_key, p2p_usdc_weth.address)
    ...
    # The exact loan the create path stored is reconstructible; assert its hash.
    expected = _expected_loan(p2p_usdc_weth, signed_offer, loan_id, now, borrower, lender, principal, collateral_amount)
    assert compute_loan_hash(expected) == p2p_usdc_weth.loans(loan_id)
...
def test_create_loan_contract_lender_validates_when_isvalidsignature_returns_magic(
    ...
    calls `isValidSignature`. Loan creation succeeds."""
    ...
    expected = _expected_loan(
        p2p_usdc_weth, signed_offer, loan_id, now, borrower, lender_contract.address, principal, collateral_amount
    )
    assert compute_loan_hash(expected) == p2p_usdc_weth.loans(loan_id)
...
def test_create_loan_reverts_if_contract_lender_isvalidsignature_returns_non_magic(
    ...
    "offer not signed by lender" (the ERC-1271 return value is honored, not trusted)."""
...
# ============== 5. EIP-7702-delegated EOA lender ==============


def test_create_loan_7702_delegated_eoa_lender_validates_via_plain_signature(
    p2p_usdc_weth, borrower, now, lender, lender_key, kyc_borrower, kyc_lender, usdc, weth, oracle
):
    """A smart-wallet EOA with an EIP-7702 delegation HAS code, so `lender.is_contract`
    is true — yet it still signs offers with its own key.
    ...
    which is only possible via the `if signer == lender: return True` plain-signature path
    checked before the `is_contract` branch.
    """
    principal = 1000 * 10**6
    offer = _base_offer(now, lender, borrower, usdc, weth, oracle, principal)
    signed_offer = sign_offer(offer, lender_key, p2p_usdc_weth.address)
