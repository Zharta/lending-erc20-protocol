...
def centrifuge_async_vault_mock(centrifuge_async_vault_mock_contract_def, usdc, weth):
    ...
    share == weth (collateral token paid on deposit claim / pulled on requestRedeem). Per Decision
    Log D24 this single vault is BOTH the market mint_addr and redemption_addr for the Centrifuge async market.
    """
...
def p2p_usdc_weth_centrifuge(
    ...
    Both mint_addr and redemption_addr point at the same CentrifugeAsyncVaultMock (D24: the Centrifuge async vault is
    stateless about the AsyncVault address and receives it from the market on each call). A non-zero
    max_pending_window (50s) is set so the borrower-only guard on cancel_pending_loan is meaningful
    (with window 0 the permissionless recovery path would always be open). The window is kept BELOW the
    async offers' 100s duration (D30 requires duration > window) so that "past window" (t=51) is
    decoupled from "past maturity" (t=101): a permissionless cancel can fire while the loan is still
    pending-but-not-defaulted.
    """
    return p2p_lending_multivault_erc20_contract_def.deploy(
        usdc,  # payment_token
        weth,  # collateral_token
        ...
        transfer_agent,  # transfer_agent
        centrifuge_async_vault_mock.address,  # mint_addr (D24: same Centrifuge vault as redemption)
        centrifuge_async_vault_mock.address,  # redemption_addr (D24)
        boa.eval("empty(address)"),  # vault_registrar_addr
...
def sign_centrifuge_offer(p2p_usdc_weth_centrifuge, usdc, weth, oracle, lender, borrower, lender_key, now):
    """Build + sign an async (Centrifuge-market) offer for the given `principal`.

    Sibling of `sign_leveraged_offer` but targets `p2p_usdc_weth_centrifuge`. Flat boilerplate only: the
    test declares `principal` and calls `create_leveraged_loan` itself.
    """
...
def fund_centrifuge_leveraged(p2p_usdc_weth_centrifuge, usdc, borrower, lender):
    ...
    Sibling of `fund_leveraged` but for the Centrifuge async market and with no vault collateral seeding (the async
    vault mints collateral later, on the deposit claim).
    """
