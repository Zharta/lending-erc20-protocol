"""
Tests verifying that MultiVault contracts do NOT support callable loans.

MultiVault contracts explicitly reject offers with non-zero call_eligibility or call_window
...
def valid_offer_usdc_weth(now, borrower, lender, oracle, lender_key, usdc, weth, p2p_usdc_weth):
    """Valid offer with call_eligibility=0 and call_window=0 (required for MultiVault)."""
    principal = 1000 * 10**6
    offer = Offer(
        principal=principal,
        apr=1000,
...
def test_create_loan_reverts_if_call_eligibility_nonzero(
    ...
    """MultiVault contracts reject offers with call_eligibility != 0."""
    principal = 1000 * 10**6
    offer = Offer(
        principal=principal,
        apr=1000,
...
def test_create_loan_reverts_if_call_window_nonzero(
    ...
    """MultiVault contracts reject offers with call_window != 0."""
    principal = 1000 * 10**6
    offer = Offer(
        principal=principal,
        apr=1000,
