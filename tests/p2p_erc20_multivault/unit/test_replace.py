...
def test_replace_loan_adds_collateral_to_escrow(
    ...
    initial_protocol_collateral = weth.balanceOf(vault_addr)

    weth.approve(vault_addr, loan.collateral_amount, sender=loan.borrower)
    p2p_usdc_weth.replace_loan(loan, offer_usdc_weth2, 0, loan.collateral_amount * 2, kyc_lender2, sender=loan.borrower)

    assert weth.balanceOf(vault_addr) == initial_protocol_collateral + loan.collateral_amount
...
def test_replace_loan_returns_collateral_to_borrower(
    ...
    assert weth.balanceOf(vault_addr) == initial_protocol_collateral - collateral_diff
...
def test_replace_loan_reverts_if_loan_redeemed(
    p2p_usdc_weth, ongoing_loan_usdc_weth, usdc, now, offer_usdc_weth2, kyc_lender2, lender2
):
    loan = ongoing_loan_usdc_weth
    ...
    with boa.reverts("loan redeemed"):
        p2p_usdc_weth.replace_loan(
            redeemed_loan, offer_usdc_weth2, 0, loan.collateral_amount, kyc_lender2, sender=loan.borrower
        )
