...
def test_settle_loan_creates_pending_transfer_on_erc20_transfer_fail(
    ...
    p2p_erc20_weth = p2p_lending_multivault_erc20_contract_def.deploy(
        erc20,
        weth,
        oracle,
        False,
...
def redeemed_loan_for_settle(
    ...
    Simulates: borrower redeems all collateral, redemption converts to payment token,
...
def test_settle_loan_reverts_if_redeem_result_bad_sig(p2p_usdc_weth, redeemed_loan_for_settle, usdc, lender_key, borrower):
    ...
    with boa.reverts("invalid redeem result sig"):
        p2p_usdc_weth.settle_loan(redeemed_loan, bad_signed, sender=redeemed_loan.borrower)


def test_settle_loan_reverts_if_redeem_result_vault_mismatch(
    p2p_usdc_weth, redeemed_loan_for_settle, usdc, owner_key, borrower
):
    ...
    with boa.reverts("redeem not concluded"):
        p2p_usdc_weth.settle_loan(redeemed_loan, signed, sender=redeemed_loan.borrower)
