...
def test_liquidate_loan_reverts_if_redeem_result_bad_sig(
    p2p_usdc_weth, redeemed_loan_with_payment, usdc, lender_key, now, borrower
):
    """Correct vault + timestamp, but result signed by a non-owner key -> sig check reverts.

    Passes the timestamp and vault guards in _is_loan_redeem_concluded so ordering
    reaches _validate_redeem_result_sig (P2PLendingMultiVaultBase.vy:425).
    """
    redeemed_loan, redeem_result, _ = redeemed_loan_with_payment

    assert redeemed_loan.redeem_start > 0  # precondition: loan is redeemed
    assert redeem_result.timestamp >= redeemed_loan.redeem_start  # precondition: passes timestamp guard
    vault_addr = p2p_usdc_weth.vault_id_to_vault(borrower, redeemed_loan.vault_id)
    assert redeem_result.vault == vault_addr  # precondition: passes vault guard

    # Sign with lender_key (non-owner) so the ecrecovered signer != self.owner
    bad_signed = sign_redeem_result(redeem_result, lender_key)

    boa.env.time_travel(seconds=redeemed_loan.maturity - now + 1)

    with boa.reverts("invalid redeem result sig"):
        p2p_usdc_weth.liquidate_loan(redeemed_loan, bad_signed, sender=redeemed_loan.lender)


def test_liquidate_loan_reverts_if_redeem_result_vault_mismatch(
    p2p_usdc_weth, redeemed_loan_with_payment, usdc, owner_key, now, borrower
):
    """Owner-signed result with a wrong vault -> _is_loan_redeem_concluded returns False.

    The vault-mismatch branch (P2PLendingMultiVaultBase.vy:606) has no message of its own;
    it flows through the _resolve_redeem_balances "redeem not concluded" assert (line 627).
    """
    redeemed_loan, redeem_result, _ = redeemed_loan_with_payment
    ...
    stranger_vault = boa.env.generate_address("stranger_vault")
    mismatched = RedeemResult(
        vault=stranger_vault,  # wrong vault
        collateral_redeemed=redeem_result.collateral_redeemed,
        payment_redeemed=redeem_result.payment_redeemed,
        timestamp=redeem_result.timestamp,
    )
    # Correct owner key, so a matching vault would conclude the redeem
    signed = sign_redeem_result(mismatched, owner_key)

    boa.env.time_travel(seconds=redeemed_loan.maturity - now + 1)

    with boa.reverts("redeem not concluded"):
        p2p_usdc_weth.liquidate_loan(redeemed_loan, signed, sender=redeemed_loan.lender)


# Aggregated-offer committed-liquidity tests.
# commited_liquidity is only ever incremented by loan.amount (principal) at creation, so
# liquidate_loan must reduce it by loan.amount, not outstanding_debt (principal + interest).
# A single-loan offer clamps at 0 and hides over-freeing; an aggregated offer (borrower == empty,
# shared tracing_id) with committed total > loan.amount exposes it, so these tests commit TWO
# loans (total 2*P) before liquidating one.


def _liquidity_key(lender: str, tracing_id: bytes) -> bytes:
    """keccak256(concat(convert(lender, bytes32), tracing_id))."""
...
def test_liquidate_loan_frees_committed_liquidity_by_principal_not_debt_on_aggregated_offer(
    ...
    """liquidate_loan reduces commited_liquidity by loan.amount (P), not outstanding_debt
    (P + interest). Uses an aggregated offer (available_liquidity = 2*P, borrower == empty) so
    the committed total is 2*P when loan A is liquidated, exposing over-freeing that a single-loan
    offer's clamp-at-0 would hide.

    Exercises the payment+collateral branch (in_vault_payment_token == 0, collateral value >= debt)
    with a third-party liquidator so the reduction runs.
    """
    ...
    # loans of principal P. Non-zero interest (apr=1000) and non-zero fees so outstanding_debt != principal.
    offer = Offer(
        principal=principal,
        apr=1000,
        payment_token=usdc.address,
    ...
    # Freed exactly loan.amount (P), not outstanding_debt (P + interest).
    assert committed_before - committed_after == principal
    assert committed_after == principal  # exactly loan B's commitment remains

    # End-to-end: exactly one more loan of principal P fits (utilization respected), a further one is rejected.
    weth.deposit(value=collateral_amount, sender=borrower)
    weth.approve(p2p_usdc_weth.wallet_to_vault(borrower), collateral_amount, sender=borrower)
    p2p_usdc_weth.create_loan(signed_offer, principal, collateral_amount, kyc_borrower, kyc_lender, sender=borrower)
    assert p2p_usdc_weth.commited_liquidity(liquidity_key) == 2 * principal
...
def test_liquidate_loan_shortfall_frees_only_recovered_principal_retaining_loss_on_aggregated_offer(
    ...
    """Shortfall branch: when a full liquidation recovers LESS than the principal (recovery < P),
    liquidate_loan frees only the recovered principal (min(lender_funds_delta, loan.amount)),
    NOT the full loan.amount. The unrecovered part is a
    ...
    committed_after = p2p_usdc_weth.commited_liquidity(liquidity_key)
    # Free only min(lender_funds_delta, loan.amount) == recovered (< P); the loss (P - recovered) stays committed.
