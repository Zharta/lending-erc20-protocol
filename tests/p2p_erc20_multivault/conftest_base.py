...
class Loan(NamedTuple):
    ...
    def get_capped_interest(self, timestamp):
        # Interest accrued to `timestamp` but never past maturity (mirrors base._compute_capped_interest,
        # the value the liquidation-style cancel_pending_loan charges).
    ...
    def get_liquidation_interest(self):
        # Full-term interest charged on liquidation (mirrors _compute_liquidation_interest).
