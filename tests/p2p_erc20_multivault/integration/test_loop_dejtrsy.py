"""Async (Centrifuge ERC-7540) leveraged-loan lifecycle against the real deJTRSY/USDC AsyncVault on an
Ethereum mainnet fork.

Reuses the deJAAA fulfilment/whitelisting recipe (same Centrifuge V3 deployment); only the token / vault /
pool-id / scid / oracle differ, and the restriction hook is read live via share.hook(). ERC-7887
cancellation resolves synchronously, so cancel_pending_loan / cancel_redeem complete in two back-to-back calls.

Collateral / mint_spend are sized from the live oracle and convertToShares; minted collateral is read from
dejtrsy.balanceOf(vault_addr) after start_loan (a hair below the fulfilled shares, from claim rounding).
"""
...
def p2p_usdc_dejtrsy(
    ...
    return p2p_lending_multivault_erc20_contract_def.deploy(
        ...
        DEJTRSY_ASYNC_VAULT,  # mint_addr (the Centrifuge AsyncVault)
        DEJTRSY_ASYNC_VAULT,  # redemption_addr
...
def test_create_start_and_settle_async_loan(
    ...
    assert redeem_started_event.collateral_token == dejtrsy.address
...
def test_async_redeem_blocks_settle_until_fulfilled(
    ...
    assert redeem_event.collateral_token == dejtrsy.address
