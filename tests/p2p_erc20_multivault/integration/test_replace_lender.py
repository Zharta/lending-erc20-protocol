...
def _calc_deltas(loan, offer, principal, timestamp, contract) -> (int, int, int, int):
    ...
    max_interest_delta_val = _max_interest_delta(loan, offer, new_principal, timestamp)
    borrower_compensation = max(max_interest_delta_val, origination_fee_amount - new_principal + outstanding_debt)
...
def test_replace_loan_lender(
    ...
    # Accrue real interest before the replace, but stay inside the real ACRED Chainlink oracle's
    # ~24h heartbeat: at the fork block the feed's answer is already ~9h old, so time-travelling a
    # full day would make latestRoundData() revert (stale) when replace_loan_lender re-reads the
    # oracle. 12h keeps the feed fresh while still accruing a non-zero interest amount.
    ...
    new_loan_id = p2p_usdc_acred.replace_loan_lender(loan, offer_usdc_acred2, offer.principal, kyc_lender2, sender=loan.lender)
    # event must be captured before any other call on this contract (overwrites _computation)
    event = get_last_event(p2p_usdc_acred, "LoanReplacedByLender")
    ...
    new_origination_fee_amount = offer.origination_fee_bps * new_principal // BPS
    ...
    new_loan = Loan(
        ...
        min_collateral_amount=offer.min_collateral_amount,
        origination_fee_amount=new_origination_fee_amount,
        protocol_upfront_fee_amount=new_protocol_upfront_fee_amount,
        protocol_settlement_fee=p2p_usdc_acred.protocol_settlement_fee(),
        partial_liquidation_fee=p2p_usdc_acred.partial_liquidation_fee(),
        full_liquidation_fee=p2p_usdc_acred.full_liquidation_fee(),
        call_eligibility=offer.call_eligibility,
    ...
    assert event.amount == new_principal
    ...
    assert event.origination_fee_amount == new_origination_fee_amount
    assert event.protocol_upfront_fee_amount == new_protocol_upfront_fee_amount
    ...
    assert event.full_liquidation_fee == p2p_usdc_acred.full_liquidation_fee()
    assert event.offer_id == compute_signed_offer_id(offer_usdc_acred2)
    ...
    assert event.paid_protocol_settlement_fee_amount == protocol_settlement_fee
    ...
    # Collateral stays in the same vault for lender-initiated refinance
