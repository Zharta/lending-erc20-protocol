"""
Integration tests for leveraged "loop" loans with the real ACRED Securitize DS token, driven
through the first-class `create_leveraged_loan` method on P2PLendingMultiVaultErc20.

These tests use the actual ACRED DS token, the Securitize on-ramp swap and the Chainlink oracle
on a mainnet fork.

NEW first-class leverage flow (no proxy, no Balancer flash loan):
1. The loan vault (P2PLendingVaultSecuritizeMV, capabilities MINT_SYNC | REDEEM_MANUAL) is created
   by the lending contract and registered as a Securitize investor via the vault registrar.
2. The lender's principal (minus origination fee) is pulled into the loan vault.
3. The borrower's margin (`mint_spend - (principal - origination_fee)`) is pulled into the loan
   vault.
4. `vault.mint_sync(...)` buys ACRED collateral straight from the vault's own USDC balance via the
   real SecuritizeSwap (instant on-ramp). No Balancer flash loan, no SecuritizeProxy.
5. The loan is built against the actual minted collateral and the principal is reconciled per
   Decision Log D13 (fixed principal -> leftover to borrower).

The retired proxy/flash-loan path (`SecuritizeProxy` + Balancer) is intentionally NOT used here.

Redeem/settle with the Securitize MV vault is manual/async: `redeem()` -> `redeem_manual` simply
transfers the redeemed ACRED to the `redemption_addr` (redemption_wallet) and ignores the oracle-rate
args (no on-chain USDC payout). The USDC proceeds arrive off-chain, which the test simulates by
transferring USDC from the redemption_wallet into the vault before settlement.
"""
...
# The acred-specific "loop ladder": principals borrowed and collateral acquired per iteration of the
# original 5-round Balancer/proxy loop. In the first-class flow the whole loop collapses into a
# single `create_leveraged_loan` call: `principal` = sum(principals), and the total collateral
# `collateral_amount` = sum(collateral_amounts) is minted in one shot. Kept here to preserve the
# original acred economics (principal ~194.3k USDC against ~261 ACRED, realized LTV ~68%).
...
def test_create_leveraged_loan(
    ...
    Borrower supplies a USDC margin, lender supplies the principal; together they buy ACRED
    collateral straight through the real SecuritizeSwap. No proxy, no flash loan.
    """
    ...
    # Conservative floor: the swap delivers ~collateral_amount (exact-rate, zero-fee), so a 1% buffer
    # absorbs integer-division rounding while still being a meaningful minimum.
    ...
    # The ACRED physically held by the loan vault IS the minted collateral (fixed principal + zero
    # fee -> no refund). This is the ground-truth collateral amount.
    ...
    # 1. State: loan hash matches the expected Loan (fixed principal -> loan amount == principal).
    loan = Loan(
        id=loan_id,
...
def test_redeem(
    ...
    """Full lifecycle on the new flow: create_leveraged_loan -> redeem -> settle.

    The Securitize MV vault's `redeem_manual` transfers the redeemed ACRED to the redemption wallet
    and keeps the residual in the vault; no on-chain USDC payout occurs. The USDC proceeds arrive
    off-chain, simulated by transferring USDC from the redemption wallet into the vault before
    settlement.
    """
    ...
    # The Securitize MV vault's redeem_manual transfers the redeemed ACRED to the redemption wallet
    # and keeps the residual in the vault; no on-chain USDC payout occurs.
    ...
    # The USDC for the redeemed collateral arrives off-chain. Simulate by transferring USDC from the
    # redemption wallet into the vault (exact-rate, zero-fee swap value of the redeemed ACRED).
