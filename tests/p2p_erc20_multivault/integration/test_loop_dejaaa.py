...
`P2PLendingMultiVaultErc20`, driven against the REAL deJAAA/USDC AsyncVault on an ETHEREUM MAINNET fork.

This is the deJAAA-on-Ethereum honest suite; its Base/deSPXA sibling (`test_loop_despxa.py`) exercises the
same `P2PLendingVaultCentrifugeAsync` impl against the real deSPXA token on a Base fork, reusing the same
chain-neutral `centrifuge_*` fulfilment recipe.

Everything async is otherwise only unit-tested against `CentrifugeAsyncVaultMock`. These fork tests validate the
assumptions a mock can't: real Centrifuge V3 fulfilment semantics (a three-message ApprovedDeposits ->
...
SYNCHRONOUSLY by the AsyncRequestManager (the cancel is claimable in the same tx the vault requests it,
so `cancel_pending_loan` / `cancel_redeem` need only two back-to-back calls, with NO issuer step between).
...
(ETH_FORK_BLOCK), chosen new enough that the deJAAA CentrifugeOracleAdapter has bytecode.
...
Sizing note: the deJAAA oracle is 18-decimal here (config says 8; the live CentrifugeOracleAdapter reports
18). deJAAA ~1.037 USDC. All collateral / mint_spend amounts are derived from the real oracle rate and the
...
`dejaaa.balanceOf(vault_addr)` after start_loan, exactly like the acred leveraged test reads its minted amount.
"""
...
def p2p_usdc_dejaaa(
    ...
    return p2p_lending_multivault_erc20_contract_def.deploy(
        ...
        DEJAAA_ASYNC_VAULT,  # mint_addr (D24: the Centrifuge AsyncVault)
        DEJAAA_ASYNC_VAULT,  # redemption_addr (D24)
