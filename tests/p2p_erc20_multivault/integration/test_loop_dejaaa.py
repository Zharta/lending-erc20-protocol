"""Async (Centrifuge ERC-7540) leveraged-loan lifecycle against the real deJAAA/USDC AsyncVault on an
Ethereum mainnet fork.

Fulfilment is impersonated at the Centrifuge spoke via the manager's callback(...) (see the dejaaa_*
fixtures and centrifuge_* helpers in conftest.py). ERC-7887 cancellation resolves synchronously here, so
cancel_pending_loan / cancel_redeem complete in two back-to-back calls.

Collateral / mint_spend are sized from the live oracle and convertToShares so realized LTV stays under the
offer cap; minted collateral is read from dejaaa.balanceOf(vault_addr) after start_loan (a hair below the
fulfilled shares, from claim rounding).
"""
...
# --- deJAAA (Ethereum mainnet) constants ------------------------------------------------------------
DEJAAA = "0xAAA0008C8CF3A7Dca931adaF04336A5D808C82Cc"  # Centrifuge share token (collateral, 18 dec)
DEJAAA_ASYNC_VAULT = "0x4865BC9701fBD1207A7B50e2aF442C7DAf154c9c"  # deJAAA/USDC ERC-7540 AsyncVault
DEJAAA_MANAGER = "0xF48256AbDDf96EcDDc4B3DbD23E8C1921f9761Ae"  # AsyncRequestManager (deposit + redeem)
DEJAAA_SPOKE = "0xEC3582fcDc34078a4B7a8c75a5a3AE46f48525aB"  # warded on the manager -> impersonate to fulfil
DEJAAA_ROOT = "0x7Ed48C31f2fdC40d37407cBaBf0870B2b688368f"  # warded on the hook -> impersonate to whitelist
DEJAAA_ORACLE = "0x5559cb9059F1a5de55E342f6A2cdb42cDAbC7409"  # CentrifugeOracleAdapter deJAAA/USD (18 dec)
DEJAAA_USDC = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"
DEJAAA_POOL_ID = 281474976710659
DEJAAA_SCID = bytes.fromhex("00010000000000030000000000000001")


# --- deJAAA fixtures (real deployed handles bound under the dir's Ethereum fork env) ------------------


@pytest.fixture
def dejaaa_keeper(accounts, borrower, lender):
...
@pytest.fixture
def dejaaa_token(centrifuge_share_token_contract_def, boa_env):
    return centrifuge_share_token_contract_def.at(DEJAAA)


@pytest.fixture
def dejaaa_oracle(oracle_contract_def, boa_env):
    return oracle_contract_def.at(DEJAAA_ORACLE)


@pytest.fixture
def dejaaa_async_vault(centrifuge_async_vault_contract_def, boa_env):
    """The real deJAAA/USDC ERC-7540 AsyncVault."""
    return centrifuge_async_vault_contract_def.at(DEJAAA_ASYNC_VAULT)


@pytest.fixture
def dejaaa_manager(centrifuge_manager_contract_def, boa_env):
    return centrifuge_manager_contract_def.at(DEJAAA_MANAGER)


@pytest.fixture
def dejaaa_hook(centrifuge_hook_contract_def, dejaaa_token):
    """The share token's CURRENTLY active restriction hook (read `share.hook()` live — it was migrated
    away from the config address). Exposes updateMember for whitelisting the loan vault."""
    return centrifuge_hook_contract_def.at(dejaaa_token.hook())


@pytest.fixture
def dejaaa_asset_id(centrifuge_spoke_contract_def, boa_env):
    return centrifuge_spoke_contract_def.at(DEJAAA_SPOKE).assetToId(DEJAAA_USDC, 0)
...
def p2p_usdc_dejaaa(
    ...
    """Centrifuge async (ERC-7540) market wired to the real deJAAA AsyncVault (Ethereum mainnet).

    The AsyncVault is both mint_addr and redemption_addr. max_pending_window (50s) < the async offers' 100s
    duration. All fees start at zero; individual tests bump them.
    """
    return p2p_lending_multivault_erc20_contract_def.deploy(
        DEJAAA_USDC,  # payment_token
        DEJAAA,  # collateral_token
        DEJAAA_ORACLE,  # oracle_addr
        ...
        DEJAAA_ASYNC_VAULT,  # mint_addr (the Centrifuge AsyncVault)
        DEJAAA_ASYNC_VAULT,  # redemption_addr
...
def _size_leverage(principal, oracle, async_vault):
    ...
    to acquire that many deJAAA shares at the vault's convertToShares rate (zero-fee, oracle-rate on-ramp).
    """
...
def test_create_start_and_settle_async_loan(
    ...
    p2p, dejaaa, oracle = p2p_usdc_dejaaa, dejaaa_token, dejaaa_oracle
    keeper = dejaaa_keeper

    principal = 1000 * 10**6
    collateral, mint_spend = _size_leverage(principal, oracle, dejaaa_async_vault)
    ...
    centrifuge_whitelist(dejaaa_hook, DEJAAA, vault_addr, DEJAAA_ROOT)
    ...
    # Capture events before the p2p calls below.
    ...
    # State: pending loan stored (start_time == 0), collateral == the caller's estimate. The loan-hash check
    # pins every field of the created loan.
    ...
    # Events: LoanCreated with start_time == 0 marks the loan pending; collateral is the caller's estimate
    # until LoanStarted reports the actual minted amount.
    ...
    assert created_event.collateral_token == dejaaa.address
    ...
    # Remaining LoanCreated fields (from the pending struct + offer; oracle rate computed independently).
    assert created_event.apr == pending.apr
    assert created_event.payment_token == pending.payment_token
    assert created_event.call_eligibility == pending.call_eligibility
    assert created_event.call_window == pending.call_window
    assert created_event.liquidation_ltv == pending.liquidation_ltv
    assert created_event.oracle_addr == pending.oracle_addr
    assert created_event.initial_ltv == pending.initial_ltv
    assert created_event.origination_fee_amount == pending.origination_fee_amount
    assert created_event.protocol_upfront_fee_amount == pending.protocol_upfront_fee_amount
    assert created_event.protocol_settlement_fee == pending.protocol_settlement_fee
    assert created_event.partial_liquidation_fee == pending.partial_liquidation_fee
    assert created_event.full_liquidation_fee == pending.full_liquidation_fee
    assert created_event.offer_tracing_id == offer.tracing_id
    assert created_event.oracle_rate_num == oracle.latestRoundData()[1]  # oracle_reverse=False -> num = answer
    assert created_event.oracle_rate_den == 10 ** oracle.decimals()
    ...
    assert dejaaa.balanceOf(vault_addr) == 0, "no shares until the deposit settles and the loan starts"
    assert dejaaa_async_vault.pendingDepositRequest(0, vault_addr) == mint_spend
    ...
    shares = dejaaa_async_vault.convertToShares(mint_spend)
    centrifuge_fulfill_deposit(
        dejaaa_manager, DEJAAA_POOL_ID, DEJAAA_SCID, dejaaa_asset_id, vault_addr, mint_spend, shares, DEJAAA_SPOKE
    )
    assert dejaaa_async_vault.pendingDepositRequest(0, vault_addr) == 0
    assert dejaaa_async_vault.claimableDepositRequest(0, vault_addr) > 0

    # ---- start_loan by a non-borrower keeper (permissionless) ----
    ...
    minted = dejaaa.balanceOf(vault_addr)
    assert minted >= min_collateral_out, "minted below the offer's minimum"
    realized_ltv = calc_ltv(principal, minted, usdc, dejaaa, oracle, oracle_reverse=False)
    ...
    redeem_started_event = get_last_event(p2p, "LoanCollateralRedeemStarted")  # before any other p2p call
    ...
    assert redeem_started_event.loan_id == loan_id
    assert redeem_started_event.borrower == borrower
    assert redeem_started_event.lender == lender
    assert redeem_started_event.collateral_token == dejaaa.address
    assert redeem_started_event.vault_id == 0
    assert redeem_started_event.redeem_start == redeem_start
    assert redeem_started_event.redeem_residual_collateral == 0
    assert compute_loan_hash(redeeming) == p2p.loans(loan_id)
    assert dejaaa_async_vault.pendingRedeemRequest(0, vault_addr) == minted
    assert dejaaa.balanceOf(vault_addr) == 0, "shares moved into the redeem request"

    redeem_assets = dejaaa_async_vault.convertToAssets(minted)  # USDC the issuer settles the redeem for
    centrifuge_fulfill_redeem(
        dejaaa_manager, DEJAAA_POOL_ID, DEJAAA_SCID, dejaaa_asset_id, vault_addr, minted, redeem_assets, DEJAAA_SPOKE
    )
    ...
    assert dejaaa_async_vault.pendingRedeemRequest(0, vault_addr) == 0
    assert dejaaa_async_vault.claimableRedeemRequest(0, vault_addr) > 0
    ...
    assert dejaaa.balanceOf(vault_addr) == 0
    assert dejaaa_async_vault.claimableRedeemRequest(0, vault_addr) == 0, "the redemption was actually claimed"
    ...
    assert paid_event.payment_token == redeeming.payment_token
    assert paid_event.paid_principal == redeeming.amount
    assert paid_event.paid_interest == interest
    assert paid_event.origination_fee_amount == redeeming.origination_fee_amount  # 0 in this happy path
    assert paid_event.protocol_upfront_fee_amount == redeeming.protocol_upfront_fee_amount  # 0 in this happy path
    ...
    assert dejaaa.balanceOf(borrower) == 0, "no residual collateral to return"
...
def test_cancel_pending_unfilled_loan(
    ...
    is synchronous: the phase-1 call that submits it also makes it claimable, so the next call runs the
    liquidation-style USDC waterfall and clears the loan. Nonzero origination + settlement + full-liquidation
    fees make the money math real; a keeper drives it post-window.
    """
    p2p, oracle = p2p_usdc_dejaaa, dejaaa_oracle
    keeper = dejaaa_keeper
    ...
    # Past the pending window, a keeper may cancel.
    ...
    assert dejaaa_async_vault.pendingDepositRequest(0, vault_addr) == 0
    assert dejaaa_async_vault.claimableCancelDepositRequest(0, vault_addr) == mint_spend

    # Waterfall: keeper fee -> protocol fee on interest -> lender recovery -> borrower surplus.
    ...
    # Pure cancel (never fulfilled): the whole estate is reclaimed as PAYMENT, split across the payment legs;
    # the collateral legs are all 0 because no shares were ever fulfilled.
    assert cancelled_event.collateral_claimed == 0
    assert cancelled_event.payment_reclaimed == mint_spend
    assert cancelled_event.liquidation_fee_payment == keeper_fee
    assert cancelled_event.protocol_fee_payment == protocol_fee
    assert cancelled_event.lender_payment == lender_recovery
    assert cancelled_event.borrower_payment == borrower_surplus
    assert cancelled_event.liquidation_fee_collateral == 0
    assert cancelled_event.protocol_fee_collateral == 0
    assert cancelled_event.lender_collateral == 0
    assert cancelled_event.borrower_collateral == 0
...
# ---------------------------------------------------------------------------
# 3. Force-unwind: a fulfilled deposit that can't start -> share-denominated split
# ---------------------------------------------------------------------------
...
def test_force_unwind_fulfilled_below_min_collateral(
    ...
    claiming the shares and splitting them oracle-valued, liquidation-style. All legs are paid in deJAAA
    ...
    low_shares = dejaaa_async_vault.convertToShares(mint_spend) * 80 // 100
    centrifuge_fulfill_deposit(
        dejaaa_manager, DEJAAA_POOL_ID, DEJAAA_SCID, dejaaa_asset_id, vault_addr, mint_spend, low_shares, DEJAAA_SPOKE
    )
    assert dejaaa_async_vault.pendingDepositRequest(0, vault_addr) == 0
    assert dejaaa_async_vault.claimableDepositRequest(0, vault_addr) > 0
    ...
    # cancel_pending_loan force-unwinds: claim the shares and split them oracle-valued, in shares. The minted
    # amount is priced on-chain per-tx, so take PendingLoanCancelled.collateral_claimed as ground truth.
    rate_num = oracle.latestRoundData()[1]
    rate_den = 10 ** oracle.decimals()
    payment_dec = 10 ** usdc.decimals()
    collateral_dec = 10 ** dejaaa.decimals()
    ...
    keeper_before = dejaaa.balanceOf(keeper)
    lender_before = dejaaa.balanceOf(lender)
    borrower_before = dejaaa.balanceOf(borrower)
    protocol_before = dejaaa.balanceOf(p2p.protocol_wallet())

    boa.env.time_travel(seconds=p2p.max_pending_window() + 1)  # keeper (permissionless) drives the unwind
    interest = loan.get_capped_interest(boa.eval("block.timestamp"))
    assert p2p.cancel_pending_loan(loan, EMPTY_MINT_RESULT, sender=keeper) is True
    liq_event = get_last_event(p2p, "PendingLoanCancelled")
    ...
    if value_after_fee >= debt:
        ...
        borrower_shares = 0
    # Scenario preconditions: a covered force-unwind that pays every leg (keeper + protocol + lender +
    ...
    # Event: every split leg matches the independent computation. Force-unwind of a FULFILLED deposit pays
    # every leg in COLLATERAL shares; nothing was reclaimed as payment, so all payment legs are 0.
    assert liq_event.id == loan_id
    assert liq_event.borrower == borrower
    assert liq_event.lender == lender
    assert liq_event.payment_reclaimed == 0
    assert liq_event.lender_collateral == lender_shares
    assert liq_event.liquidation_fee_collateral == liquidation_fee_shares
    assert liq_event.protocol_fee_collateral == protocol_fee_shares
    assert liq_event.borrower_collateral == borrower_shares
    assert liq_event.lender_payment == 0
    assert liq_event.liquidation_fee_payment == 0
    assert liq_event.protocol_fee_payment == 0
    assert liq_event.borrower_payment == 0
    assert liq_event.caller == keeper

    # Balances: legs paid in deJAAA shares, plus share conservation.
    assert dejaaa.balanceOf(keeper) == keeper_before + liquidation_fee_shares
    assert dejaaa.balanceOf(lender) == lender_before + lender_shares
    assert dejaaa.balanceOf(p2p.protocol_wallet()) == protocol_before + protocol_fee_shares
    assert dejaaa.balanceOf(borrower) == borrower_before + borrower_shares
...
def test_async_redeem_blocks_settle_until_fulfilled(
    ...
    centrifuge_fulfill_deposit(
        ...
        vault_addr,
        mint_spend,
        dejaaa_async_vault.convertToShares(mint_spend),
        DEJAAA_SPOKE,
    )
    p2p.start_loan(pending, EMPTY_MINT_RESULT, 0, sender=keeper)
    minted = dejaaa.balanceOf(vault_addr)
    ...
    assert redeem_event.borrower == borrower
    assert redeem_event.lender == lender
    assert redeem_event.collateral_token == dejaaa.address
    assert redeem_event.vault_id == 0
    assert redeem_event.redeem_start == redeem_start
    assert redeem_event.redeem_residual_collateral == residual
    assert dejaaa_async_vault.pendingRedeemRequest(0, vault_addr) == redeemed_shares
    assert dejaaa.balanceOf(vault_addr) == residual, "residual stays in the loan vault"
    ...
    redeem_assets = dejaaa_async_vault.convertToAssets(redeemed_shares)
    centrifuge_fulfill_redeem(
        dejaaa_manager, DEJAAA_POOL_ID, DEJAAA_SCID, dejaaa_asset_id, vault_addr, redeemed_shares, redeem_assets, DEJAAA_SPOKE
    )
    ...
    assert expected_lender_payment + protocol_fee + surplus == claimed_usdc, "USDC conservation"
    assert dejaaa.balanceOf(borrower) == residual, "residual collateral returned to the borrower"
    assert dejaaa.balanceOf(vault_addr) == 0
    assert p2p.commited_liquidity(compute_liquidity_key(lender, offer.tracing_id)) == 0
...
def test_cancel_redeem_restores_active_loan(
    ...
    assert dejaaa_async_vault.claimableCancelRedeemRequest(0, vault_addr) == minted, "cancel already claimable"
    ...
    assert compute_loan_hash(started) == p2p.loans(loan_id)
    assert dejaaa.balanceOf(vault_addr) == minted, "all redeemed shares reclaimed into the loan vault"
    assert dejaaa_async_vault.pendingRedeemRequest(0, vault_addr) == 0
