"""Async (Centrifuge ERC-7540) leveraged-loan lifecycle against the real deSPXA/USDC AsyncVault on a Base
mainnet fork.

Reuses the deJAAA fulfilment/whitelisting recipe (Base shares Ethereum's Centrifuge V3 addresses); only the
token / vault / pool-id / scid / asset-id / oracle differ. Unlike Ethereum, ERC-7887 cancellation resolves
ASYNCHRONOUSLY: the issuer must relay a fulfilment before the reclaim is claimable, so cancel_pending_loan /
cancel_redeem submit in one call and complete in a later one.

The Base RPC is derived from BOA_FORK_RPC_URL (eth-mainnet -> base-mainnet); the module skips if that
substring is absent (see base_boa_env). Price (~757 USDC/deSPXA) is read live via the CentrifugeOracleAdapter.
"""
...
def p2p_usdc_despxa(
    ...
    All deployments (facets, kyc, vault impl, oracle adapter, market) must live under the Base env — a market
    wired to another env's facet addresses delegatecalls into empty code and silently no-ops. The AsyncVault
    is both mint_addr and redemption_addr. max_pending_window (50s) < the async offers' 100s duration. All
    fees start at zero; individual tests bump them.
    """
    return p2p_lending_multivault_erc20_contract_def.deploy(
        ...
        DESPXA_ASYNC_VAULT,  # mint_addr (the Centrifuge AsyncVault)
        DESPXA_ASYNC_VAULT,  # redemption_addr
...
def test_create_start_and_settle_async_loan(
    ...
    assert redeem_started_event.collateral_token == despxa.address
...
def test_cancel_pending_unfilled_loan(
    ...
    """Create -> cancel_pending_loan (deposit never filled). On Base the ERC-7887 deposit cancellation is
    ASYNCHRONOUS (unlike Ethereum's synchronous cancel): the phase-1 call only submits the cancel (reclaimed
    payment not yet claimable). The issuer must relay a FulfilledDepositRequest with cancelledAssets ==
    mint_spend before the reclaimed payment is claimable; only then does the next call run the
    liquidation-style USDC waterfall and clear the loan. Nonzero origination + settlement + full-liquidation
    ...
    # Phase 1: submit the cancel. On Base this only submits it (async cancel) — the deposit is still pending
    # and the reclaimed payment is not yet claimable, so the call returns False.
...
def test_async_redeem_blocks_settle_until_fulfilled(
    ...
    assert redeem_event.collateral_token == despxa.address
...
def test_cancel_redeem_restores_active_loan(
    ...
    """A started loan enters redemption, then cancel_redeem reverses it. On Base the redeem cancellation is
    ASYNCHRONOUS (unlike Ethereum): cancel_redeem phase-1 only submits the cancel (shares not yet claimable).
    The issuer must relay a FulfilledRedeemRequest with cancelledShares == minted before the reclaimed shares
    are claimable; only then does the next cancel_redeem call reclaim the shares, reset redeem_start, and
    return the loan to its normal active state with all shares back in the vault.
    """
    p2p, despxa, oracle, usdc = p2p_usdc_despxa, despxa_token, despxa_oracle, base_usdc
    borrower, lender, keeper = base_borrower, base_lender, base_keeper
    kyc_validator_contract = base_kyc_validator_contract
