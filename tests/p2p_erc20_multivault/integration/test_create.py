...
def test_create_loan(
    ...
    # The borrower (Securitize investor) authorizes the connector to register its per-loan vault.
    ...
    protocol_wallet_balance_before = usdc.balanceOf(p2p_usdc_acred.protocol_wallet())
    ...
    # MultiVault-specific event fields
    ...
    # Payment token: borrower receives principal minus origination fee, lender pays principal plus origination fee
    ...
    # Protocol wallet receives upfront fee
    protocol_upfront_fee_amount = p2p_usdc_acred.protocol_upfront_fee() * principal // BPS
    assert usdc.balanceOf(p2p_usdc_acred.protocol_wallet()) == protocol_wallet_balance_before + protocol_upfront_fee_amount
...
def test_create_loan_registers_vault_with_registrar(
    ...
    # Precondition: the vault is not yet registered until the borrower authorizes + a loan is created.
    vault_addr = p2p_usdc_acred.vault_id_to_vault(borrower, vault_id)
