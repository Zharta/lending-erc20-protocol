...
def test_replace_loan(
    ...
    # full day would make latestRoundData() revert (stale) when replace_loan re-reads the oracle.
    # 12h keeps the feed fresh while still accruing a non-zero interest amount.
    ...
    if delta_borrower < 0:
        usdc.approve(p2p_usdc_acred.address, -delta_borrower, sender=loan.borrower)
    ...
    initial_protocol_collateral = acred.balanceOf(vault_addr)
    ...
    # Collateral: vault has old + new additional collateral
    assert acred.balanceOf(vault_addr) == initial_protocol_collateral + new_collateral_amount - loan.collateral_amount
    assert acred.balanceOf(loan.borrower) == initial_borrower_collateral - new_collateral_amount + loan.collateral_amount
