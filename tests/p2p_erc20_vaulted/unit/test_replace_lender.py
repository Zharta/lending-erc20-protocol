...
def test_replace_loan_lender_with_undercollateralized_ltv(
    p2p_usdc_weth, ongoing_loan_usdc_weth, lender2_key, usdc, now, kyc_lender2, lender2, weth, oracle
):
    loan = ongoing_loan_usdc_weth
...
def test_replace_loan_lender_current_ltv_uses_outstanding_debt(
    ...
    # The new offer's max_iltv must accommodate the resulting initial_ltv
    # initial_ltv = ltv(collateral_amount, new_principal=outstanding_debt) = ltv_outstanding
    ...
    new_offer = Offer(
        ...
        payment_token=usdc.address,
        collateral_token=weth.address,
        duration=365 * DAY,  # Must be >= old loan remaining duration for repayment_time check
        origination_fee_bps=0,
        min_collateral_amount=0,
        ...
        call_eligibility=0,
        call_window=0,
        liquidation_ltv=new_max_iltv + 1000,  # Must be > max_iltv
        oracle_addr=oracle.address,
