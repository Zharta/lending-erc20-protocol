...
def p2p_usdc_acred(
    ...
    contract = p2p_lending_erc20_contract_def.deploy(
        ...
        vault_registrar.address,  # vault_registrar
    )
...
def sec_borrower(securitize_registry, p2p_usdc_acred, securitize_owner, now):
    """Register the vault under the borrower's existing investor ID."""
    wallet = "0x81aF1E160c290E8Fff6381CCF67981f012Cf1009"
    vault_addr = p2p_usdc_acred.wallet_to_vault(wallet)

    # Get the borrower's existing investor ID from mainnet data
    borrower_investor_id = securitize_registry.getInvestor(wallet)

    securitize_registry.updateInvestor(
        borrower_investor_id,
        "",
        "PT",
        [vault_addr],
        [SEC_REG_ACCREDITED],
