...
def test_transfer_loan_reverts_if_kyc_validation_fails(
    p2p_usdc_weth, ongoing_loan_usdc_weth, transfer_agent, kyc_validator_contract, kyc_validator_key, lender_key, now
):
    new_borrower = boa.env.generate_address("new_borrower")
    wrong_wallet = boa.env.generate_address("wrong_wallet")

    invalid_kyc_list = [
        sign_kyc(wrong_wallet, now, kyc_validator_key, kyc_validator_contract.address),
        sign_kyc(new_borrower, now, lender_key, kyc_validator_contract.address),
        sign_kyc(new_borrower, now, kyc_validator_key, boa.env.generate_address("random")),
    ]

    for kyc in invalid_kyc_list:
        with boa.reverts("KYC validation fail"):
            p2p_usdc_weth.transfer_loan(ongoing_loan_usdc_weth, new_borrower, kyc, sender=transfer_agent)
...
def test_transfer_loan_deletes_old_loan(
    p2p_usdc_weth, ongoing_loan_usdc_weth, transfer_agent, kyc_for, kyc_validator_contract
):
    new_borrower = boa.env.generate_address("new_borrower")
    new_borrower_kyc = kyc_for(new_borrower, kyc_validator_contract.address)
