...
- ``build_erc20_contract_def_with_log_stuff``: generates the ``log_stuff`` dummy
  function boa needs to decode facet-emitted events. boa fails to register an event
  declared in the main contract but only ``log``-emitted from a delegatecall facet,
  so we append a function that logs every such event once. Generating it from
  ``used_events`` / ``event_defs`` keeps it in sync when an event field is added.
"""
...
# Single Ethereum-mainnet fork block for ALL suites forking BOA_FORK_RPC_URL.
# >= the CentrifugeOracleAdapter deploy (~25022228) so Centrifuge fork flows work.
ETH_FORK_BLOCK = 25400000

# Single BASE-mainnet fork block for the deSPXA suite (test_loop_despxa.py), which forks a
# base-mainnet RPC derived from BOA_FORK_RPC_URL. One block per chain, same rule as ETH_FORK_BLOCK.
# Chosen where the deSPXA AsyncVault prices and the Centrifuge spoke are live, so a freshly-deployed
# CentrifugeOracleAdapter can read pricePoolPerShare for deSPXA.
