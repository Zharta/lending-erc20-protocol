...
def start_loan(
    ...
    if (vault_capabilities & base.MINT_ASYNC) != 0:
        ...
        no_reclaim: uint256 = 0  # no cancel requested or claimable
        minted, no_reclaim = extcall _vault.claim_mint(base.mint_addr, True, False)
...
@pure
@internal
def _take_from_payment_then_collateral(target_value: uint256, pay: uint256, col: uint256, rate: base.UInt256Rational, payment_token_decimals: uint256, collateral_token_decimals: uint256) -> (uint256, uint256):

    # Take up to `target_value` (in payment units) from the estate, drawing from `pay` first and then from `col` shares. Returns (payment_taken, shares_taken).
    # Shares are rounded down when converting value to shares

    payment_taken: uint256 = min(pay, target_value)
    remaining_value: uint256 = target_value - payment_taken
    if remaining_value == 0 or col == 0:
        return payment_taken, 0

    col_value: uint256 = col * rate.numerator * payment_token_decimals // (rate.denominator * collateral_token_decimals)
    take_value: uint256 = min(remaining_value, col_value)
    shares: uint256 = take_value * rate.denominator * collateral_token_decimals // (rate.numerator * payment_token_decimals)
    return payment_taken, min(shares, col)
...
def cancel_pending_loan(
    ...
    @notice Cancel a pending (async) leveraged loan, unwinding the in-flight ERC-7540 deposit.
            Returns True if the cancellation completed and the loan was settled, False if it must be retried.
            If the deposit is cleanly fulfilled with no cancellation, a startable loan cannot be cancelled.
    @dev State machine over the vault `mint_status()`. A Centrifuge deposit can be partially fulfilled
         and partially cancelled simultaneously, so request/cancel pending/claimable are independent
         counters, not mutually exclusive. Order of resolution:
           - cancel pending  -> a cancellation is still settling; retry later (return False).
           - request pending -> the deposit is still in-flight; request its cancellation (cancelDepositRequest)
                                and retry once it settles (return False).
           - terminal (both pendings zero): claim whatever is claimable, distribute the combined estate in order
                                (caller fee > protocol fee > lender recovery > borrower) and delete the loan (return True).
    ...
    if status.cancel_pending > 0:
        # cancellation was submitted but has not resolved yet, retry later
        return False
    if status.request_pending > 0:
        # deposit is still ongoing, request cancellation and retry once it settles
        extcall _vault.cancel_mint(base.mint_addr)
        return False

    # pre: request_pending == 0 and cancel_pending == 0
    minted: uint256 = 0
    reclaimed: uint256 = 0
    minted, reclaimed = extcall _vault.claim_mint(base.mint_addr, status.request_claimable > 0, status.cancel_claimable > 0)

    assert minted > 0 or reclaimed > 0, "no pending mint"

    # startability guard applies only to a fulfilled deposit with no cancellation
    ...
    # payment then collateral, priority order: caller fee > protocol fee > lender > borrower
    convertion_rate: base.UInt256Rational = base._get_oracle_rate(oracle_addr, oracle_reverse)
    interest: uint256 = base._compute_capped_interest(loan)
    debt: uint256 = (loan.amount - loan.origination_fee_amount) + interest

    col_value: uint256 = minted * convertion_rate.numerator * payment_token_decimals // (convertion_rate.denominator * collateral_token_decimals)
    estate_value: uint256 = reclaimed + col_value

    fee_value: uint256 = min(debt * loan.full_liquidation_fee // BPS, estate_value)
    value_after_fee: uint256 = estate_value - fee_value
    protocol_value: uint256 = min(loan.protocol_settlement_fee * interest // BPS, value_after_fee)
    lender_value: uint256 = (debt - protocol_value) if value_after_fee >= debt else (value_after_fee - protocol_value)

    payment_amount: uint256 = reclaimed
    collateral_amount: uint256 = minted

    caller_pay: uint256 = 0
    caller_col: uint256 = 0
    caller_pay, caller_col = self._take_from_payment_then_collateral(fee_value, payment_amount, collateral_amount, convertion_rate, payment_token_decimals, collateral_token_decimals)
    payment_amount -= caller_pay
    collateral_amount -= caller_col

    protocol_pay: uint256 = 0
    protocol_col: uint256 = 0
    protocol_pay, protocol_col = self._take_from_payment_then_collateral(protocol_value, payment_amount, collateral_amount, convertion_rate, payment_token_decimals, collateral_token_decimals)
    payment_amount -= protocol_pay
    collateral_amount -= protocol_col

    lender_pay: uint256 = 0
    lender_col: uint256 = 0
    lender_pay, lender_col = self._take_from_payment_then_collateral(lender_value, payment_amount, collateral_amount, convertion_rate, payment_token_decimals, collateral_token_decimals)
    payment_amount -= lender_pay
    collateral_amount -= lender_col

    borrower_pay: uint256 = payment_amount
    borrower_col: uint256 = collateral_amount

    base.loans[loan.id] = empty(bytes32)
    base._reduce_commited_liquidity(
        loan.lender,
        loan.offer_tracing_id,
    ...
    if reclaimed > 0:
        extcall _vault.withdraw_funds(payment_token, reclaimed)

    base._send_funds(canceller, caller_pay, payment_token)
    base._send_collateral(canceller, caller_col, _vault)
    base._send_funds(base.protocol_wallet, protocol_pay, payment_token)
    base._send_collateral(base.protocol_wallet, protocol_col, _vault)
    base._send_funds(loan.lender, lender_pay, payment_token)
    base._send_collateral(loan.lender, lender_col, _vault)
    base._send_funds(loan.borrower, borrower_pay, payment_token)
    base._send_collateral(loan.borrower, borrower_col, _vault)
    ...
    log main.PendingLoanCancelled(
        ...
        payment_reclaimed=reclaimed,
        lender_payment=lender_pay,
        lender_collateral=lender_col,
        liquidation_fee_payment=caller_pay,
        liquidation_fee_collateral=caller_col,
        protocol_fee_payment=protocol_pay,
        protocol_fee_collateral=protocol_col,
        borrower_payment=borrower_pay,
        borrower_collateral=borrower_col,
        caller=canceller,
    )
    return True
