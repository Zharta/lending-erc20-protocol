...
def claim_mint(mint_vault: address, claim_deposit: bool, claim_cancel: bool) -> (uint256, uint256):
    ...
    @dev Two independent claim legs driven by the lending contract once mint_status reports the
         corresponding amounts as claimable (either or both may be requested in one call):
    ...
    @return A tuple (shares, reclaimed): the collateral shares minted+credited by the deposit and the
            payment reclaimed by the cancellation.
    """
...
def claim_redeem(redemption_vault: address, claim_redeem: bool, claim_cancel: bool) -> (uint256, uint256):
    ...
    @dev Mirror of claim_mint. Two independent claim legs driven by the lending contract once
         redeem_status reports the corresponding amounts as claimable (either or both in one call):
    ...
    @return A tuple (assets, shares): the payment assets received by the redemption and the collateral
            shares reclaimed by the cancellation.
    """
