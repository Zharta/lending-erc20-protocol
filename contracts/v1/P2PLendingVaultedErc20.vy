...
VERSION: public(constant(String[30])) = "P2PLendingErc20.20260226"
...
def __init__(
    ...
    if _vault_registrar_addr != empty(address):
        assert staticcall base.VaultRegistrar(_vault_registrar_addr).token() == collateral_token, "vault registrar token mismatch"
...
def change_vault_registrar(new_vault_registrar: address):
    ...
    if new_vault_registrar != empty(address):
        assert staticcall base.VaultRegistrar(new_vault_registrar).token() == collateral_token, "vault registrar token mismatch"
