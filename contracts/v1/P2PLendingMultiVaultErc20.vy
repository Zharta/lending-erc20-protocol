...
"""
@title P2PLendingMultiVaultErc20
...
event MintAddressChanged:
    old_addr: address
    new_addr: address

event RedemptionAddressChanged:
    old_addr: address
    new_addr: address
...
event PendingLoanCancelled:
    ...
    lender_payment: uint256
    lender_collateral: uint256
    liquidation_fee_payment: uint256
    liquidation_fee_collateral: uint256
    protocol_fee_payment: uint256
    protocol_fee_collateral: uint256
    borrower_payment: uint256
    borrower_collateral: uint256
    caller: address
...
def __init__(
    ...
    base.mint_addr = _mint_addr
    base.redemption_addr = _redemption_addr
...
@external
def set_redemption_addr(_address: address):

    """
    @notice Set redemption wallet
    @dev Sets the redemption contract / wallet to the given address. Admin function.
    @param _address The address of the new redemption contract / wallet.
    """

    assert msg.sender == base.owner
    log RedemptionAddressChanged(old_addr=base.redemption_addr, new_addr=_address)
    base.redemption_addr = _address

@external
def set_mint_addr(_address: address):

    """
    @notice Set mint wallet
    @dev Sets the mint contract / wallet to the given address. Admin function.
    @param _address The address of the new mint contract / wallet.
    """

    assert msg.sender == base.owner
    log MintAddressChanged(old_addr=base.mint_addr, new_addr=_address)
    base.mint_addr = _address
