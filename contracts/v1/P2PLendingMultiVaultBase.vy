...
"""
@title P2PLendingMultiVaultBase
...
interface Vault:
    ...
    def claim_mint(mint_vault: address, claim_deposit: bool, claim_cancel: bool) -> (uint256, uint256): nonpayable
    ...
    def claim_redeem(redemption_vault: address, claim_redeem: bool, claim_cancel: bool) -> (uint256, uint256): nonpayable
...
mint_addr: public(address)
redemption_addr: public(address)
...
@internal
def _send_collateral(wallet: address, _amount: uint256, _vault: Vault):
    if _amount == 0:
        return
...
@internal
def _receive_collateral(_from: address, _amount: uint256, _vault: Vault):
...
def _resolve_redeem_balances(loan: Loan, _vault: Vault, payment_token: address, redeem_result: SignedRedeemResult, vault_capabilities: uint256) -> (uint256, uint256):
    if (vault_capabilities & REDEEM_ASYNC) != 0:
        status: AsyncStatus = staticcall _vault.redeem_status(self.redemption_addr)
        assert status.request_pending == 0 and status.cancel_pending == 0, "redeem not settled"
        claimed: uint256 = 0
        reclaimed: uint256 = 0
        claimed, reclaimed = extcall _vault.claim_redeem(self.redemption_addr, status.request_claimable > 0, status.cancel_claimable > 0)
        if status.request_claimable > 0:
            return claimed, loan.redeem_residual_collateral + reclaimed
        else:
            # Already-claimed case for transferred loans
            return staticcall IERC20(payment_token).balanceOf(_vault.address), loan.redeem_residual_collateral + reclaimed
...
@internal
@view
def _get_vault(wallet: address, vault_id: uint256, _vault_impl_addr: address) -> Vault:
...
@internal
def _create_vault_by_id_if_needed(wallet: address, vault_impl_addr: address, payment_token: address, vault_id: uint256, vault_registrar: address) -> Vault:
    ...
    if not _vault.is_contract:
        ...
        extcall Vault(_vault).initialise(wallet, payment_token)
...
@internal
def _create_vault_if_needed(wallet: address, vault_impl_addr: address, payment_token: address, vault_registrar: address) -> Vault:
    return self._create_vault_by_id_if_needed(wallet, vault_impl_addr, payment_token, self.vault_count[wallet], vault_registrar)


@internal
def _create_new_vault(wallet: address, vault_impl_addr: address, payment_token: address, vault_registrar: address) -> Vault:
    # only creates a vault if needed
    _vault: Vault = self._create_vault_if_needed(wallet, vault_impl_addr, payment_token, vault_registrar)
